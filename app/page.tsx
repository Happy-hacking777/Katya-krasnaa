  ...
  return (
    <div className="min-h-screen bg-white p-8 overflow-hidden">
            ...
            {artworks.map((artwork, index) => (
              <motion.img
                ...
                className="absolute rounded-full object-cover
                          w-24 h-24 sm:w-32 sm:h-32 lg:w-40 lg:h-40" // Responsive sizes
                initial={{
                  x: 0,
                  y: 0,
                  scale: 0,
                ...
                animate={{
                  x: Math.cos(index * (Math.PI * 2 / artworks.length)) * (window.innerWidth < 640 ? 100 : 200), // Smaller on mobile
                  y: Math.sin(index * (Math.PI * 2 / artworks.length)) * (window.innerWidth < 640 ? 100 : 200), // Adjust based on screen width
                  scale: 1,
                  rotate: 360,
                  transition: {
                    duration: 2,
      ...
      <motion.div
        ...
        className="max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-2 gap-16" // 2 columns and more gap
      >
        {artworks.map((artwork, index) => (
          <motion.div
            key={artwork.id}
      ...
      <footer className="mt-16 bg-gray-100 p-8 rounded-lg text-center">
        ...
        <p className="text-lg text-gray-700">Email: <a href={`mailto:${artist.email}`} className="text-primary hover:underline">{artist.email}</a></p>
        <p className="text-lg text-gray-700">Phone: <a href={`tel:${artist.phone}`} className="text-primary hover:underline">{artist.phone}</a></p>
      </footer>
    </div>
  )
}